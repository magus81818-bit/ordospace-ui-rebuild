...
function clientStepProgressHtml(){const p=clientProjectProgress();const steps=[['스코핑','✓','완료',100],['계약','✓','완료',100],['제작','◉',p.pct+'%',p.pct],['납품','○','대기',0]];return '<div class="ordo-client-step-grid grid grid-cols-4 gap-2" role="list" aria-label="프로젝트 단계">'+steps.map((s,i)=>{const state=i<2?'complete':(i===2?'current':'pending');return '<div class="ordo-client-step-card rounded-xl border '+(i<2?'border-st-okbd bg-st-okbg':(i===2?'border-st-pendbd bg-st-pendbg':'border-bd-default bg-bg-secondary'))+' p-3" role="listitem" data-state="'+state+'"'+(state==='current'?' aria-current="step"':'')+'><div class="flex items-center justify-between gap-2 text-[12px] font-semibold"><span>'+moduleEsc(s[0])+'</span><span aria-hidden="true">'+moduleEsc(s[1])+'</span></div><div class="mt-2 text-[18px] font-semibold tabular '+(i<2?'text-st-okfg':(i===2?'text-st-pendfg':'text-tx-tertiary'))+'">'+moduleEsc(s[2])+'</div><div class="mt-2 progress-track" role="progressbar" aria-label="'+moduleEsc(s[0])+' 진행률" aria-valuemin="0" aria-valuemax="100" aria-valuenow="'+s[3]+'"><div class="progress-fill" style="width:'+s[3]+'%"></div></div></div>';}).join('')+'</div>';}
...
function renderApprovals(){
  ...
  if(left)left.innerHTML=cards.map(c=>'<button type="button" data-approval-card="'+moduleEsc(c.id)+'" aria-pressed="'+String(c.id===_approvalSelectedCardId)+'" class="ordo-client-approval-row w-full text-left p-4 hover:bg-bg-secondary '+(c.id===_approvalSelectedCardId?'bg-bg-secondary':'')+'"><div class="flex items-start justify-between gap-3"><div class="min-w-0"><div class="text-[13px] font-semibold text-tx-primary truncate">'+moduleEsc(c.module)+'</div><div class="text-[11px] text-tx-tertiary mt-1 truncate">'+moduleEsc(c.spec)+' · '+moduleEsc(c.gateRef)+' · '+moduleEsc(c.completedAt||c.createdAt||'-')+'</div></div>'+window.ORDO_UI_COMPONENTS.StatusBadge(moduleLabel(c.status),'warn')+'</div></button>').join('')||'<div class="ordo-c-empty-state p-6 text-center text-[13px] font-semibold text-st-okfg">모든 항목을 확인했습니다 ✓</div>';
