...
const evidenceRoot = path.join(root, 'evidence', 'redo', 'r04');
const artifactRoot = path.join(root, 'artifacts', 'redo', 'r04');
const baseUrl = 'http://127.0.0.1:4179/';
const tokenHrefs = ['dashboard-salesops.tokens.css', 'dashboard-salesops.primitives.css'];
const viewports = {
...
async function setTokenSheetDisabled(page, disabled) {
  await page.evaluate(({ tokenHrefs, disabled }) => {
    const links = Array.from(document.querySelectorAll('link[rel="stylesheet"]')).filter(node => tokenHrefs.some(href => node.href.includes(href)));
    if (links.length !== tokenHrefs.length) throw new Error(`dashboard stylesheets missing: ${links.length}/${tokenHrefs.length}`);
...
async function setDescendantSheetsDisabled(page, disabled) {
  await page.evaluate(disabledValue => {
    const laterSheets = ['dashboard-salesops.shell.css','dashboard-salesops.admin.css','dashboard-salesops.client.css','dashboard-salesops.worker.css'];
